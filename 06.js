...
function func1(input) {
  ...
  input.split('|')
    .map(instr => {
      const [_, action, x1, y1, x2, y2] = regex.exec(instr)
      let x, y

      for (y = +y1; y <= +y2; y += 1) {
        for (x = +x1; x <= +x2; x += 1) {
          const value = lights[y][x]
          let newValue
...
if (require.main === module) {
  console.log('Part 1 (how many lights are on):', func1(input))
  console.log('Part 2 (how bright are the lights):', func2(input))
}
