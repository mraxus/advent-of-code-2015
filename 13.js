...
function happyTSP(graph, person, unseated, firstPerson) {
  if (unseated.length === 0) return graph[person][firstPerson] || 0

  return unseated
    .map((nextLocation, index) => {
      const pastHappi = happyTSP(
        graph,
        nextLocation,
        unseated.slice(0, index).concat(unseated.slice(index + 1)),
        firstPerson
      )
      const happi = graph[person][nextLocation]
      return happi + pastHappi
    })
    .reduce((maxHappi, happi) => Math.max(maxHappi, happi), 0)
}

function func1(input) {
  const happinessGraph = input.split('|')
    .map(x => x.replace('happiness units by sitting next to ', ''))
    .map(x => x.replace('would lose ', '-'))
    .map(x => x.replace('would gain ', ''))
    .map(x => x.replace('.', '').split(' '))
    .reduce((graph, [a, dist, b]) => {
      graph[a] = graph[a] || {}
      graph[b] = graph[b] || {}

      graph[a][b] = graph[a][b] || 0
      graph[b][a] = graph[b][a] || 0
  ...
  const unseated = Object.keys(happinessGraph)

  return unseated
    .map((person, index) => happyTSP(
      happinessGraph,
      person,
      unseated.slice(0, index).concat(unseated.slice(index + 1)),
      person
...
if (require.main === module) {
  console.log('Part 1 (biggest happiness):', func1(input))
  console.log('Part 2 (biggest happiness with me):', func2(input))
}
