...
function func1(input, char='a') {
  ...
  const instructions = input.split('|')
    .map(instr => {
      ...
      if (len === 1) {
        let x = parts[0]
        rhs = `get('${x}')`
        if (isVariable(x)) { variables.push(x) }
      } else if (len === 2) {
        let x = parts[1]
        rhs = `~get('${x}')`
        if (isVariable(x)) { variables.push(x) }
      } else {
        const [x, operator, y] = parts
        switch (operator) {
          case 'AND':    rhs = `get('${x}') & get('${y}')`; break
          case 'OR':     rhs = `get('${x}') | get('${y}')`; break
          case 'LSHIFT': rhs = `get('${x}') << get('${y}')`; break
          case 'RSHIFT': rhs = `get('${x}') >> get('${y}')`; break

          default:
            throw new Error(`operator unknown: ${operator}`)
        }

        if (isVariable(x)) { variables.push(x) }
        if (isVariable(y)) { variables.push(y) }
      }

      return {
        instruction: `${lhs} = (${rhs}) & 65535`,
        processed: false,
        variables,
      }
    })
  ...
  // Run instructions as variables are initialized
  let len = instructions.length
  let processed = 0
  let index = 0
  let item = instructions[0]
  while (processed < len) {
    if (!item.processed) {
      if (item.variables.filter(x => registry[x] === undefined).length === 0) {
        eval(item.instruction)
        item.processed = true
        processed += 1
      }
    }
    ...
    index = (index + 1)
    item = instructions[index % len]
  }

  return registry[char]
}

function func2(input, char='a') {
  return func1(input.split('|')
    .map(instr => instr.endsWith('-> b') ? `${func1(input)} -> b` : instr)
    .join('|'), char)
}

if (require.main === module) {
  console.log('Part 1 (finding a):', func1(input))
  console.log('Part 2 (finding a after setting a -> b):', func2(input))
}
